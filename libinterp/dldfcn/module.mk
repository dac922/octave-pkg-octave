...
dldfcn/$(am__leading_dot)urlwrite.oct-stamp: dldfcn/urlwrite.la
	rm -f $(<:.la=.oct)
	la=$(<F) && \
	  of=$(<F:.la=.oct) && \
	  cd dldfcn && \
...
dldfcn_urlwrite_la_SOURCES = dldfcn/urlwrite.cc
dldfcn/urlwrite.df: CPPFLAGS += $(CURL_CPPFLAGS)
dldfcn_urlwrite_la_CPPFLAGS = $(AM_CPPFLAGS) $(CURL_CPPFLAGS)
dldfcn_urlwrite_la_LDFLAGS = -avoid-version -module $(NO_UNDEFINED_LDFLAG) $(CURL_LDFLAGS) $(OCT_LINK_OPTS)
dldfcn_urlwrite_la_LIBADD = $(DLD_LIBOCTINTERP_LIBADD) ../liboctave/liboctave.la $(CURL_LIBS) $(OCT_LINK_DEPS)
